...
class MiddlewareRunner {
    ...
    static async run(appInstance, controller, route, argsBuilderBuilderArgs, middlawareType) {
        ...
        for (const routeMiddleware of routeMiddlewares) {
            ...
            for (const appMiddleware of appInstance.__middlawares) {
                ...
                if (middlewareOriginalConstructor !== appMiddleware.__originalConstructor || appMiddleware.type !== middlawareType)
                    continue;
    ...
    static runRequestMiddlewares(appInstance, controller, route, argsBuilderBuilderArgs) {
        return this.run(appInstance, controller, route, argsBuilderBuilderArgs, middleware_type_1.MiddlewareType.request);
    }
    static runResponseMiddlewares(appInstance, controller, route, argsBuilderBuilderArgs) {
        return this.run(appInstance, controller, route, argsBuilderBuilderArgs, middleware_type_1.MiddlewareType.response);
    }
}
